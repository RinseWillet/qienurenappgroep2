const interneMedewerkerNaam = document.getElementById("interne-medewerker-naam");
...
window.onload = () => {
    interneMedewerkerNaamFunction();
    aanpassenurl();
};

const interneMedewerkerNaamFunction = () => {
    ...
    xhr.onreadystatechange = function () {
        if (xhr.readyState == 4) {
            interneMedewerker = JSON.parse(this.responseText);

            let archiefFormulierHTML = ``;
            let huidigMaandFormulier = ``;

            interneMedewerkerNaam.innerHTML = `${interneMedewerker.naam}`;

            var formulieren = interneMedewerker.archief;
            var huidigFormulier = interneMedewerker.tijdelijkeFormulieren;

            console.log(interneMedewerker.archief);
            ...
            if (huidigFormulier.length > 0) {
                for (let x = 0; x < huidigFormulier.length; x++) {
                    ...
                    if ((e.adminStatus === 'OPEN' && e.opdrachtgeverStatus === 'OPEN') && e.ingezondenFormulier == true) {
                        ...
                        formulierBeoordeling = "Wachten op oedkeuring";
                    ...
                    huidigMaandFormulier = `<a href="./medewerkerformulier?id=${urlId}&formulierid=${e.id}" class="list-group-item list-group-item-action d-flex justify-content-between" id="${e.id}"><span id="${e.id}">${maand}</span><span id="${e.id}">${e.jaar}</span><span id="${e.id}">${formulierLocatie}</span><span id="${e.id}">${formulierBeoordeling}</span><i id="${e.id}" class="far fa-eye"></i></a>`;
...
const genereerFormulier = (formulier) => {
    ...
    modalHeader.innerHTML = `${interneMedewerker.naam} | ${formulier.maand}/${formulier.jaar}`
